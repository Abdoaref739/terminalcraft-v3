...
def login():
        ...
        answer1 = questionary.text("Enter ur UserName, please!!").ask()
        answer2 = questionary.password("Enter ur Password, please!!").ask()
...
def options():
    ...
    choices=["Create directory", "access my files", "create new file", "store a code snippet", "execute a code","Encrypt some data","Decrypt some data" , "reach some documentations", "Using the ChatBot!"],
        ).ask()

        encryption_list = ['a','b', 'c', 'd', 'e']
        if program_options == "Create directory":
                ...
                if os.path.exists(file_name):
                        ...
                        file_name = input("write the directory name")
                else:
    ...
    choices=["json", "txt"],
        ...
        elif program_options == "Encrypt some data":
                data = input('Enter ur data to start the encryption process!')
                for i in range(len(data)):
                        encrypted_data_list = data[i]
                        encrypted_data = encrypted_data_list.replace(data[i], encryption_list[i + 1])
                        print(Panel(f'[bold green]{encrypted_data}[/bold green]'))
                return_back()


        elif program_options == "Decrypt some data":
                data = input('Enter ur data to start the encryption process!')
                for i in range(len(data)):
                        encrypted_data_list = data[i]
                        encrypted_data = encrypted_data_list.replace(data[i], encryption_list[-i + 1])
        ...
        elif program_options == "Using the ChatBot!":
                import requests
                def req_answer():
                        val = input("write a message")
                        response = requests.post(
                                "https://ai.hackclub.com/chat/completions",
                                json={"messages": [{"role": "user", "content": val}]}
                        )
                        data = response.json()
                        global response_message
                        response_message = data["choices"][0]["message"]["content"]
                        print("GPT:", response_message)
                req_answer()
                return_back()
